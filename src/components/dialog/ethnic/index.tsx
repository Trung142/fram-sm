...
mutation Add($input: EthnicInput!) {
  addEthnic(data: $input) {
    id
    name
  }
}`;
...
mutation Update($input: EthnicInput!) {
  updateEthnic(data: $input) {
    id
    name
  }
}`;
...
const EthnicDialog = (props: EthnicDialogProps) => {
  const { data, type, open, onSubmit } = props;
  const [show, setShow] = useMemo(() => open, [open]);
  const prefix = usePrefix("ethnic");
...
EthnicDialog.defaultProps = {
  type: 'add',
}
export default EthnicDialog;
