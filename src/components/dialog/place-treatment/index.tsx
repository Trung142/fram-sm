...
mutation Add($input: OldPlaceTreatmentInput!) {
  addOldPlaceTreatment(data: $input) {
    id
    name
  }
}`;
...
mutation Update($input: OldPlaceTreatmentInput!) {
  updateOldPlaceTreatment(data: $input) {
    id
    name
  }
}`;
...
const OldPlaceTreatmentDialog = (props: Props) => {
  const { data, type, open, onSubmit } = props;
  const [show, setShow] = useMemo(() => open, [open]);
  const prefix = usePrefix("old_place_treatment");
          ...
          <FormControl fullWidth>
            ...
            {errors.name && (
              <FormHelperText sx={{ color: 'error.main' }} id='validation-basic-first-name'>
                Tên bệnh viện/phòng khám không được để trống
              </FormHelperText>
            )}
          </FormControl>
...
OldPlaceTreatmentDialog.defaultProps = {
  type: 'add',
}

export default OldPlaceTreatmentDialog;
