...
const DetailDialog = (props: Props) => {
  ...
  const COLUMN_DEF: GridColDef[] = [
    ...
    },
    {
      ...
      minWidth: 100,
      field: 'price',
      headerName: 'Giá chuyển',
      renderCell: params => {
        return `${formatVND(params.row?.product.price)} `
      }
    },
    {
      flex: 0.3,
  ...
  return (
    <MUIDialog open={props.open} maxWidth='xl' title='Thông tin phiếu xuất điều chuyển'>
      <>
          ...
          <Grid item md={8}>
            <StyledDataGrid>
              <DataGrid
                ...
                rows={(props.data?.whTransferDts ?? []).map((item: any, index: number) => ({
                  ...item,
                ...
                <Stack direction={'column'} gap={3}>
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography display={'flex'} alignItems={'center'}>
                      <PersonOutlineOutlinedIcon /> {props.data?.whPersion.fristName} {props.data?.whPersion.lastName}
                    </Typography>
                  ...
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography>Nơi xuất</Typography>
                    <Typography>{props.data?.placeRelease?.name}</Typography>
                  </Box>
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography>Kho xuất</Typography>
                    <Typography>{props.data?.whRelease?.name}</Typography>
                  </Box>
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography>Trạng thái nơi xuất</Typography>
                    {!props.data?.statusPlaceRelease ? (
                      <span className={styles.temp}>CHỜ XỬ LÝ</span>
                    ) : (
                  ...
                  <Box display={'flex'} justifyContent={'space-between'}>
                    ...
                    <Typography>{props.data?.placeReceiving?.name}</Typography>
                  </Box>
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography>Kho nhận</Typography>
                    <Typography>{props.data?.whReceiving?.name}</Typography>
                  </Box>
                  <Box display={'flex'} justifyContent={'space-between'}>
                    <Typography>Trạng thái nơi nhận</Typography>
                    {!props.data?.statusPlaceReceiving? (
                      <span className={styles.temp}>CHỜ XỬ LÝ</span>
                    ) : (
                        <span className={styles.completed}>HOÀN THÀNH</span>
                    )}
                  ...
                  <Box display={'flex'} justifyContent={'space-between'}>
                    ...
                    <Typography>{formatVND(props.data?.totalAmount)}</Typography>
                  </Box>
                  <Box display={'flex'} justifyContent={'space-between'}>
                ...
                <Box display={'flex'} flexDirection={'column'} gap={5}>
                  ...
                  >
                    <Box display={'flex'} gap={2}>
                      ...
                      <Typography display={'flex'} alignItems={'center'}>
                        {props.data?.whPersion.fristName} {props.data?.whPersion.lastName} -{' '}
                    ...
                    <Box display={'flex'} justifyContent={'space-between'}>
                      ...
                      <Typography>{props.data?.wh?.name}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Nhà cung cấp</Typography>
                      <Typography>{props.data?.supplier?.name}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                    ...
                    <Box display={'flex'} justifyContent={'space-between'}>
                      ...
                      <Typography fontWeight={'bold'}>{formatVND(props.data?.totalAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Giảm giá</Typography>
                      <Typography>{formatVND(props.data?.totalDiscount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Tổng VAT</Typography>
                      <Typography>{formatVND(props.data?.totalVatAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography fontWeight={'bold'}>Thành tiền</Typography>
                      <Typography fontWeight={'bold'}>{formatVND(props.data?.finalAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Thanh toán</Typography>
                      <Typography>{formatVND(props.data?.totalPaid)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography fontWeight={'bold'}>Công nợ</Typography>
                      <Typography fontWeight={'bold'}>{formatVND(props.data?.debt)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Ghi chú</Typography>
                      <Typography>{props.data?.note}</Typography>
        ...
        <Stack direction={'row'} sx={{ backgroundColor: '#8EAFB7' }} spacing={2} justifyContent={'flex-end'} p={5}>
          ...
          <Button
            ...
            onClick={() => route.push(`/inventory/ep_transfer/edit/${props.data?.id}`)}
          >
