...
const AddWhExistence = () => {
  const [addWExistence] = useMutation(ADD_WH_EXISTENCE)
  ...
  const productDataIp: any[] = useMemo(() => {
    return getProductIp?.getWhExistence?.items[9].whExistenceDts ?? []
  }, [getProductIp])
  ...
  const handleAddIpinventory = async (status: string) => {
    ...
    if (batch) {
      toast.error('Bạn chưa chọn mã số lô')
      return
    } else {
      ...
      addWExistence({
        variables: {
          input: dataWhExistence
        }
      }).then((res: any) => {
        const dataWhExistenceDt = selectedProducts.map((data: any) => {
          return {
            unitId: data.unitId,
            whExistenceId: res.data.addWhExistence.id,
        ...
        addWhExistenceDt({
          variables: {
            input: JSON.stringify(dataWhExistenceDt)
          },
          onError: () => {
