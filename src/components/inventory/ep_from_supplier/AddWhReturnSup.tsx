...
import { ADD_RETURN_SUPPLIER, ADD_MANY_RETURN_SUPPLIER_DT, ADD_CANSALE, ADD_MANY_CANSALE, UPDATE_CANSALE , UPDATE_MANY_CANSALE } from './graphql/mutation'
import { /*IpInventory,*/ EpFromSupplier, Product, ProductGroup, RequestType, ReturnSupInput, CanSale, ICanSale } from './graphql/variables'
    ...
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Grid item xs={4}>
            <h2 style={{ textAlign: 'left', textTransform: 'uppercase' }}>Xuất trả nhà cung cấp</h2>
          </Grid>
          <Grid item xs={8}>
      ...
      <CustomGrid container display='flex' flexDirection='row'>
        ...
        <CustomGrid item xs={6.0} display='flex' alignItems='center' >
          ...
          <Autocomplete
            ...
            renderInput={(params: any) => <TextField {...params} placeholder='Nhà cung cấp' InputLabelProps={{ shrink: true }} />}
          />
          {/* <CustomTextField
            variant='filled'
            type='number'
              ...
              {selectedProducts.map((product: any, index) => (
                <React.Fragment key={index}>
                  <TableRow key={product.id}>
                    ...
                    <StyledTableCell style={{ width: '15%' }}>                                                      {/* THÀNH TIỀN*/}
