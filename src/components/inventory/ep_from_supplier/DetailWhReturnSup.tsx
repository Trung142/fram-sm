...
import { GET_WH_EP_RETURN_SUPPLIER_DT } from './graphql/query'
...
const DetailWhReturnDialog = (props: Props) => {
  ...
  const { data: getWhReturnSupDt, loading, refetch } = useQuery(GET_WH_EP_RETURN_SUPPLIER_DT, {
    variables: {
      input: {
        whReturnSupId: { eq: props.id }
      },
  ...
  // const data = useMemo(() => {
  //   return getWhReturnSupDt?.getWhReturnSupDt?.items ?? []
  // }, [getWhReturnSupDt])
  const data = getWhReturnSupDt?.getWhReturnSupDt?.items ?? []
  ...
  return (
    <MUIDialog open={props.open} maxWidth='xl' title='Thông tin phiếu xuất'>
      <>
          ...
          <Grid item md={8}>
            <StyledDataGrid>
              <DataGrid
                ...
                rowCount={getWhReturnSupDt?.getWhReturnSupDt?.totalCount ?? 0}
                loading={loading}
                  ...
                  <Stack direction={'column'} gap={2}>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography display={'flex'} alignItems={'center'}>
                        <PersonOutlineOutlinedIcon /> {data[0]?.whReturnSup?.whPersion.fristName}{' '}
                        {data[0]?.whReturnSup?.whPersion.lastName}
                      </Typography>
                      <Typography>{moment(data[0]?.whReturnSup?.createAt).format('DD/MM/YYYY HH:mm')}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Mã phiếu</Typography>
                      <Typography>{data[0]?.whReturnSupId}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Kho nhập</Typography>
                      <Typography>{data[0]?.whReturnSup?.wh?.name}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Nhà cung cấp</Typography>
                      <Typography>{data[0]?.whReturnSup?.supplier?.name}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Trạng thái</Typography>
                      {data[0]?.whReturnSup?.status === '301' ? (
                        <Button
                          variant='outlined'
                          color='primary'
                    ...
                    <Box display={'flex'} justifyContent={'space-between'}>
                      ...
                      <Typography fontWeight={'bold'}>{formatVND(data[0]?.whReturnSup?.totalAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Giảm giá</Typography>
                      <Typography>{formatVND(data[0]?.whReturnSup?.totalDiscount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Tổng VAT</Typography>
                      <Typography>{formatVND(data[0]?.whReturnSup?.totalVatAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography fontWeight={'bold'}>Thành tiền</Typography>
                      <Typography fontWeight={'bold'}>{formatVND(data[0]?.whReturnSup?.finalAmount)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Thanh toán</Typography>
                      <Typography>{formatVND(data[0]?.whReturnSup?.totalRefund)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography fontWeight={'bold'}>Công nợ</Typography>
                      <Typography fontWeight={'bold'}>{formatVND(data[0]?.whReturnSup?.finalAmount-data[0]?.whReturnSup?.totalRefund)}</Typography>
                    </Box>
                    <Box display={'flex'} justifyContent={'space-between'}>
                      <Typography>Ghi chú</Typography>
                      <Typography>{data[0]?.whReturnSup?.note}</Typography>
                    </Box>
                  </Stack>
                )}
              </TabPanel>
                  ...
                  {/* history row 1 */}
                    ...
                    >
                      <Box display={'flex'} gap={2}>
                        ...
                        <Typography display={'flex'} alignItems={'center'}>
                          {data[0]?.whReturnSup?.whPersion.fristName} {data[0]?.whReturnSup?.whPersion.lastName} -{' '}
                          {moment(data[0]?.whReturnSup?.createAt).format('DD/MM/YYYY HH:mm')}
                        </Typography>
                      </Box>
                      <Box display={'flex'} justifyContent={'space-between'}>
                        <Typography>Mã phiếu</Typography>
        ...
        <Stack direction={'row'} sx={{ backgroundColor: '#8EAFB7' }} spacing={2} justifyContent={'flex-end'} p={5}>
          ...
          <Button
            variant='contained'
            color='primary'
            disabled={data[0]?.whReturnSup?.status === '303'}
            onClick={() => route.push(`/inventory/ep-from-supplier/edit/${props?.id}`)}
          >
...
export default DetailWhReturnDialog
