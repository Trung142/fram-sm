...
import { GET_WH_IP_FROM_SUPPLIER, GET_WH } from './graphql/query'
import moment from 'moment'
import { useRouter } from 'next/router'
import { whImportSup } from './graphql/variables'
import { formatVND } from 'src/utils/formatMoney'
import toast from 'react-hot-toast'
import { Icon } from '@iconify/react'
import { UPDATE_WH_IMPORT_SUP } from './graphql/mutation'
...
const IpInventoryFromSupplier = () => {
  ...
  const {
    ...
    error,
    refetch
  } = useQuery(GET_WH_IP_FROM_SUPPLIER, {
  ...
  const whImportSup: whImportSup[] = useMemo(() => {
    refetch()
    return getWhImportSup?.getWhImportSup?.items ?? []
  }, [getWhImportSup])
  const getWh = useMemo(() => {
  ...
  return (
    <>
      <DetailDialog open={[openDetailDialog, setOpenDetailDialog]} id={id} setPrintOpen={setPrintOpen}/>
      <Grid container spacing={2}>
        <Card sx={{ width: '100%', p: 5 }}>
          <Grid item xs={12}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ textAlign: 'center', textTransform: 'uppercase' }}>NHẬP HÀNG TỒN Từ nhà cung cấp</h2>
              <Box sx={{ display: 'flex', gap: '11px' }}>
                <Button
                  variant='contained'
                  sx={{ pl: 5, pr: 8, backgroundColor: '#0292B1', paddingX: 5 }}
                  onClick={() => route.push('/inventory/ip-from-supplier/add')}
                  startIcon={<AddIcon />}
                >
                  Thêm mới
                </Button>
      ...
      <Grid item xs={12} sx={{ mt: 3 }}>
        <Card>
          <>
            <DataGrid
              columns={COLUMN_DEF}
              rows={whImportSup.map((item: any, index: any) => ({
                ...item,
                index: index + 1 + paginationModel.page * paginationModel.pageSize
              }))}
              rowCount={getWhImportSup?.getWhImportSup?.totalCount ?? 0}
        ...
        <PrintsComponent
          printFunctionId='pr10000008'
          printType='p_wh_import_sup_id'
          printTypeId={id ?? ''}
          clinicId={user.clinicId}
          parentClinicId={user.parentClinicId}
          openPrint={printOpen}
          setOpenButtonDialog={setPrintOpen}
          titlePrint='In phiếu nhập tồn từ NCC'
        />
      </Grid>
    </>
  )
}
export default IpInventoryFromSupplier
