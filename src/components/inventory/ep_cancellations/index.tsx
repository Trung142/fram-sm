...
import { GET_WH_EP_RETURN_SUPPLIER, GET_WH_CANCEL , GET_WH } from './graphql/query'
import moment from 'moment'
import { ReturnSupInput } from './graphql/variables'
import { UPDATE_RETURN_SUPPLIER } from './graphql/mutation'
import { left } from '@popperjs/core'
import DetailWhReturnDialog from './DetailEpCancellations';
...
const EpCancel = () => {
  ...
  const COLUMN_DEF: GridColDef[] = [
...

    {
      ...
      valueGetter: params => {
        return `${formatVND(params.row.totalAmount)} `
      }
      ...
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ textAlign: 'center', textTransform: 'uppercase' }}>Xuất hủy</h2>
            <Box sx={{ display: 'flex', gap: '11px' }}>
              <Link href={'/inventory/ep-cancellations/add'}>
              <Button
                variant='contained'
