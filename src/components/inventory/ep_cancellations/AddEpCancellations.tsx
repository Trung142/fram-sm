...
import { GET_WH, 
  ...
  GET_SUPPLIER, 
  GET_PAYMENT_TYPE, 
  GET_CANSALE } from './graphql/query'

import { ADD_WH_CANCEL, ADD_MANY_CANCEL_DT, ADD_CANSALE, ADD_MANY_CANSALE, UPDATE_CANSALE , UPDATE_MANY_CANSALE } from './graphql/mutation'
import { EpCancel, EpFromSupplier, Product, ProductGroup, RequestType, ReturnSupInput, CanSale, ICanSale } from './graphql/variables'
...
const AddEpCancellations = () => {
  const [AddEpCancel, { data, loading, error }] = useMutation(ADD_WH_CANCEL)
  const [AddEpCancelDt] = useMutation(ADD_MANY_CANCEL_DT)
  ...
  const [addData, setAddData] = useState<EpCancel>({
  ...
  const checkTotalRemainingByBatchId = (batchId: string, product: any) => {
    ...
    product.cansales.forEach((data: ICanSale) => {
      if (data.batchId === batchId && data.whId === addData.whId) 
         result = data.totalRemaining
    }) 
    return result
  }
  ...
  const checkBatchByProductId = (productId: string, batch: any) => {
    ...
    batch.cansales.forEach((data: ICanSale) => {
      if (data.productId === productId && data.whId === addData.whId) 
  ...
  const handleAddEpCancel = async (status: string) => {
    const dataEpCancel = {
      whPersionId: user.id,
    ...
    selectedProducts.forEach((product: Product) => {
      ...
      product.cansales.forEach((data: CanSale) => {
        if (data.batchId === product.batchId && data.whId === addData.whId) 
          return cansale = data
      }) 
    ...
    if (dataEpCancel.whId === '' || dataEpCancel.whId === undefined || dataEpCancel.whId === null) {
      ...
      AddEpCancel({
        ...
        const dataEpCancelDt = selectedProducts.map((data: any) => {
          return {
            unitId: data.unitId,
            whExpCancelId: res.data.AddEpCancel?.id,
            // importPrice: data.price,
            quantity: data.quantity,
            // dueDate: data.dueDate,
            // finalAmount: Tongthanhtien,
            totalAmount: Tongthanhtien,
        ...
        AddEpCancelDt({
          variables: {
            input: JSON.stringify(dataEpCancelDt)
          },
          onError: () => {
  ...
  const handleSelectProduct = (selectedProduct: any) => {
    ...
    if (productIndex !== -1) {
      ...
      updateProduct[productIndex].thanhtien = ((updateProduct[productIndex].quantity || 0)) * ( selectedProduct.price + selectedProduct.price * selectedProduct.vat )
    ...
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Grid item xs={4}>
            <h2 style={{ textAlign: 'left', textTransform: 'uppercase' }}>Xuất hủy</h2>
          </Grid>
          <Grid item xs={8}>
            <Box sx={{ display: 'flex', gap: '8px', float: 'right' }}>
              ...
              >
                <Link href='/inventory/ep-cancellations' style={{ color: 'white', textDecoration: 'none' }}>
                  Quay lại
                </Link>
              </Button>
              <Box sx={{ display: 'flex' }}>
              ...
              {selectedProducts.map((product: any, index) => (
                <React.Fragment key={index}>
                  <TableRow key={product.id}>
                    ...
                    <StyledTableCell style={{ width: '20%' }}>{product.productName}</StyledTableCell>               {/* TÊN HÀNG HÓA	*/}
                    <StyledTableCell style={{ width: '20%' }}>                                                      {/* SỐ LÔ	*/}
                      {/* <Autocomplete
                        autoHighlight
                        openOnFocus
                    ...
                    <StyledTableCell style={{ width: '15%' }}>                                                      {/* ĐƠN VỊ TÍNH	*/}
                      <Autocomplete
                        fullWidth
                        autoHighlight
                        id='combo-box-demo'
                    ...
                    <StyledTableCell style={{ width: '80px' }}>                                                     {/* SỐ LƯỢNG	*/}
                      <TextField
                        id='outlined-basic'
                        variant='outlined'
                    ...
                    <StyledTableCell style={{ width: '20%' }}>                                                      {/* GIÁ NHẬP	*/}
                      <Typography>{formatVND(product.price)}</Typography>
                    </StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>0</StyledTableCell>                                   {/* CHIẾT KHẤU	*/}
                    <StyledTableCell style={{ width: '15%' }}>{product.vat ? product.vat : 0}</StyledTableCell>     {/* % VAT	*/}
                    <StyledTableCell style={{ width: '15%' }}>                                                      
...
export default AddEpCancellations
