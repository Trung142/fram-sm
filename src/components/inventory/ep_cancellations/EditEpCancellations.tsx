...
import { CanSale, ReturnSupInput, Product, ReturnSupInputDt, EpFromSupplier } from './graphql/variables'
...
import {
  ...
  DELETE_RETURN_SUPPLIER_DT,
  UPDATE_MANY_CANSALE,
  UPDATE_MANY_RETURN_SUPPLIER_DT,
  UPDATE_RETURN_SUPPLIER
...
const EditWhReturnSup = (props: Props) => {
  ...
  const [updateManyWhReturnSupDt] = useMutation(UPDATE_MANY_RETURN_SUPPLIER_DT)
  const [deleteWhReturnSupDt] = useMutation(DELETE_RETURN_SUPPLIER_DT)
  const [addManyWhReturnSupDt] = useMutation(ADD_MANY_RETURN_SUPPLIER_DT)
  ...
  const {
    ...
    refetch,
    loading: loadingWh
  } = useQuery(GET_WH_EP_RETURN_SUPPLIER, { variables: { input: { id: { eq: props.id } } } })
  ...
  const [baseWhReturnDt, setBaseWhReturnDt] = useState< ReturnSupInputDt[]>([])
  const [dataEdit, setDataEdit] = useState<ReturnSupInput>()
  ...
  useEffect(() => {
    ...
    setDataEdit(getWhReturnSup?.getWhReturnSup?.items[0])
    const whReturnSupDt: ReturnSupInputDt[] = getWhReturnSup?.getWhReturnSup?.items[0]?.whReturnSupDts
    if (whReturnSupDt && whReturnSupDt.length > 0) {
      const whReturnSupDtData: any[] = whReturnSupDt.map((e: any) => ({
        id: e.id,
        unitId: e.unitId,
        whReturnSupId: e.whReturnSupId,
      ...
      setSelectedProducts(whReturnSupDtData)
      setBaseWhReturnDt(whReturnSupDtData)
    }
  }, [getWhReturnSup])
  useEffect(() => {
    checkBatchId()
  ...
  const checkBatchId = () => {
    selectedProducts.forEach((data: ReturnSupInputDt) => {
  ...
  let unsuitableProduct: any[] = []
  const handleEditWhReturnSup = (status: string) => {
    const dataWhReturnSup = {
      ...
      totalRefund: dataEdit?.totalRefund,
      supplierId: dataEdit?.supplierId,
      paymentTypeId: dataEdit?.paymentTypeId,
      // debt:
      //   (dataEdit?.finalAmount || 0) - (dataEdit?.totalRefund || 0) < 0
      //     ? 0
      //     : (dataEdit?.finalAmount || 0) - (dataEdit?.totalRefund || 0),
      note: dataEdit?.note,
    ...
    selectedProducts.forEach((whReturnSupDt: any) => {
      let cansale: any;
      whReturnSupDt.product.cansales?.forEach((data: CanSale) => {
        if (data.batchId === whReturnSupDt.batchId && data.whId === dataEdit?.whId) 
          return cansale = data
      }) 
      if (cansale?.totalRemaining == 0 || cansale?.totalRemaining == null || cansale?.totalRemaining == undefined) {
        unsuitableProduct.push(whReturnSupDt)
      }
      else if (cansale?.totalRemaining < whReturnSupDt.quantity) {
        unsuitableProduct.push(whReturnSupDt)
      }
    })
    ...
    if (dataWhReturnSup.whId === '' || dataWhReturnSup.whId === undefined || dataWhReturnSup.whId === null) {
      ...
      unsuitableProduct.forEach(i => {
        toast.error('Mã hàng ' + i.product.id + ' không đủ số lượng để xuất trả')
      })
      ...
      updateWhReturnSup({
        variables: {
          input: JSON.stringify(dataWhReturnSup)
        },
        onError: () => {
          toast.error('Có lỗi khi cập nhật thông tin phiếu xuất kho')
        }
      })
        .then(res => {
          const existProduct: ReturnSupInputDt[] = selectedProducts.filter(item => item.id !== '')
          const newProduct: ReturnSupInputDt[] = selectedProducts.filter(item => item.id === '')
          const deleteProduct: ReturnSupInputDt[] = baseWhReturnDt.filter(
          ...
          if (existProduct.length > 0) {
            ...
            updateManyWhReturnSupDt({
              variables: {
                input: JSON.stringify(dataUpdate)
              },
              onError: () => {
                toast.error('Có lỗi xảy khi cập nhập chi tiết phiếu xuất kho')
              }
            })
          }
          if (newProduct.length > 0) {
            ...
            addManyWhReturnSupDt({
              variables: {
                input: JSON.stringify(dataNew)
              },
              onError: () => {
                toast.error('Có lỗi xảy ra khi thêm chi tiết phiếu xuất kho mới')
              }
            })
          }
          if (deleteProduct.length > 0) {
            deleteProduct.map((data: ReturnSupInputDt) => {
              deleteWhReturnSupDt({
                variables: {
                  input: data.id
                },
                onError: () => {
                  toast.error('Có lỗi xảy ra khi xóa chi tiết phiếu xuất kho mới')
                }
              })
            })
          }
              ...
              try {
                ...
                if (canSale.length > 0) {
                  if (dataEdit?.status === '302') {
                    const quantity = () => {
                      const base = baseWhReturnDt.find(item => item.productId === product.productId)
                      return base?.quantity || 0
                    }
                    const data = {
                      id: canSale[0].id,
                      quantity:    canSale[0].quantity + quantity() - product.quantity ,
                      totalRemaining:   canSale[0].totalRemaining + quantity() - product.quantity 
                    }
        ...
        .catch(() => {
          toast.error('Có lỗi khi cập nhật chi tiết phiếu xuất kho')
        })
        .then(() => {
          toast.success(`Cập nhật phiếu xuất kho với mã: ${dataEdit?.id} thành công`)
          refetch({
            input: { id: { eq: props.id } }
          })
        })
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Grid item xs={4}>
            <h2 style={{ textAlign: 'left', textTransform: 'uppercase' }}>
              SỬA PHIẾU XUẤT HÀNG CHO NHÀ CUNG CẤP
            </h2>
          </Grid>
          <Grid item xs={8}>
            <Box sx={{ display: 'flex', gap: '8px', float: 'right' }}>
              <Button
                ...
                onClick={() => route.push('/inventory/ep-from-supplier/')}
      ...
      <CustomGrid container display='flex' flexDirection='row'>
        ...
        <CustomGrid item xs={3.0} display='flex' alignItems='center'>
          ...
          <CustomTextField
            ...
            aria-readonly
            inputProps={{ readOnly: true }}
            value={changePaid === true ? dataEdit?.finalAmount : formatVND(Math.round(dataEdit?.finalAmount  || 0))}
            onChange={e => {
      ...
      <CustomGrid container display='flex' flexDirection='row'>
        <CustomGrid item xs={3.0} display='flex' alignItems='center'>
          ...
          <CustomTextField
            variant='filled'
            fullWidth
            value={changePaid === true ? dataEdit?.totalRefund : formatVND(dataEdit?.totalRefund || 0)}
            onClick={() => setChangePaid(true)}
            onBlur={() => setChangePaid(false)}
            onChange={e => {
              handleEdit('totalRefund', parseInt(e.target.value))
            }}
          />
        </CustomGrid>
      ...
      <Grid item xs={12}>
        <TableContainer component={Paper}>
          ...
          >
            ...
            <TableBody>
              {selectedProducts.map((whReturnSupDt: any, index) => (
                <React.Fragment key={index}>
                  <TableRow key={whReturnSupDt.productId}>
                    <StyledTableCell style={{ width: '10%' }}>{`${index + 1}`}</StyledTableCell>
                    <StyledTableCell style={{ width: '10%' }}>{whReturnSupDt.productId}</StyledTableCell>
                    <StyledTableCell style={{ width: '20%' }}>{whReturnSupDt.product.productName}</StyledTableCell>
                      ...
                      {dataEdit?.status === '301' ? (
                        ...
                        <InputBatchSearch
                            onChange={value => {
                              ...
                              const serviceIndex = newServices.findIndex(s => s.productId === whReturnSupDt.productId)
                              if (value && value.id) {
                                newServices[serviceIndex] = {
                                  ...newServices[serviceIndex],
                                  batchId: value.id,
                                  dueDate: whReturnSupDt.product.batches.find((i: any) => i.id === value.id)?.endDate ?? ''
                                }
                            ...
                            product={whReturnSupDt.product}
                            batchId={whReturnSupDt.batchId}
                          />

                      ) : (
                        whReturnSupDt.batchId
                      )}
                    </StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>
                      {checkTotalRemainingByBatchId(whReturnSupDt.batchId, whReturnSupDt.product)}
                    </StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>
                      <Autocomplete
                        fullWidth
                        ...
                        options={unitName}
                        style={{ minWidth: 150 }}
                        value={unitName.find((x: any) => x.id === whReturnSupDt.unitId) ?? null}
                        onChange={(e: any, value: any) => {
                          ...
                          if (value && value.id) {
                            ...
                            setSelectedProducts(newServices)
                            console.log(whReturnSupDt.product.unitId)
                          }
                        }}
                    ...
                    <StyledTableCell style={{ width: '80px' }}>
                      <TextField
                        id='outlined-basic'
                        variant='outlined'
                        value={whReturnSupDt.quantity}
                        type='number'
                        style={{ width: '70px' }}
                        inputProps={{ readOnly: checkSale(whReturnSupDt.batchId, whReturnSupDt.product) }}
                        onChange={e => {
                          const newServices = [...selectedProducts]
                          const serviceIndex = newServices.findIndex(s => s.productId === whReturnSupDt.productId)
                          newServices[serviceIndex] = {
                            ...
                            finalAmount:
                              ...
                              (whReturnSupDt.importPrice + (whReturnSupDt.importPrice * whReturnSupDt.vat) / 100)
                              
                          }

                          setSelectedProducts(newServices)
                          console.log(whReturnSupDt.finalAmount)
                        }}
                      />
                    </StyledTableCell>

                    <StyledTableCell style={{ width: '20%' }}>
                      <Typography>{formatVND(whReturnSupDt.importPrice || 0)}</Typography>
                    </StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>0</StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>
                      {whReturnSupDt.vat ? whReturnSupDt.vat : 0}
                    </StyledTableCell>
                    <StyledTableCell style={{ width: '15%' }}>
                      {formatVND(whReturnSupDt.finalAmount || 0)}
                    </StyledTableCell>
                    <TableCell style={{ width: '10%' }}>
                      <IconButton
                        ...
                        aria-label='delete'
                        onClick={() => handleRemoveProduct(whReturnSupDt.productId, index)}
                      >
...
export default EditWhReturnSup
